...
import org.example.drafts.random_saccades.RandomForegroundPosition;
...
public class RandomSaccadesAlgorithm {
        private ArrayList<UnidentifiedObject> unObjects;
        private FGPositionSequentialSampler fGPositionSampler;

        public ArrayList<UnidentifiedObject> getAllUnObjects(Mat frame) {
        unObjects = new ArrayList<UnidentifiedObject>();
        ...
        while(!fGPositionSampler.isEmpty()) {
            ...
            UnidentifiedObject unObj = new UnidentifiedObject(objColorBRG, objContour);
            unObjects.add(unObj);
